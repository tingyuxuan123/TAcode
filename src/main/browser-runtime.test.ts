...
it.each([false, true])("advertises and executes browser tools, recovering from an external opener: %s", async (attemptExternal) => {
  ...
  const server = createServer(async (request, response) => {
    ...
    const externalStep = attemptExternal && calls.length === 1;
    const navigateStep = calls.length === (attemptExternal ? 2 : 1);
    const usesTool = externalStep || navigateStep;
    const delta = usesTool
      ? { role: "assistant", tool_calls: [{ index: 0, id: `browser-test-${calls.length}`, type: "function", function: {
        name: externalStep ? "exec_command" : "browser_navigate",
        arguments: JSON.stringify(externalStep ? { cmd: `'${fakeOpen.replaceAll("'", "'\\''")}' 'http://localhost:9001/unibest/'` } : { url: "http://localhost:9001/unibest/" }),
      } }] }
    ...
    response.end(event({ id: "test", object: "chat.completion.chunk", choices: [{ index: 0, delta, finish_reason: null }] }) +
      event({ id: "test", object: "chat.completion.chunk", choices: [{ index: 0, delta: {}, finish_reason: usesTool ? "tool_calls" : "stop" }] }) + "data: [DONE]\n\n");
  });
  ...
  try {
    ...
    await host.request("prompt", { message: "打开项目web端" });
