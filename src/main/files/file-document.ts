...
import { DOCUMENT_EDIT_BYTES, type DocumentReadRequest, type DocumentWriteRequest, type FileDocument, type FileMetadata } from "../../shared/files";
...
export class FileDocuments {
  ...
  async read(request: DocumentReadRequest): Promise<FileDocument> {
    const offset = request.offset ?? 0; const length = request.length ?? DOCUMENT_EDIT_BYTES;
    ...
    try {
      ...
      const head = Buffer.alloc(Math.min(size, 3));
      ...
      return { kind: "document", projectRoot: bound.projectRoot, path: bound.path,
        ...
        metadata: { size, readBytes: slice.length, mode: Number(stat.mode) & 0o7777, bom, lineEnding: content === null ? "none" : lineEnding(content), writable, encoding,
          offset: start + from, nextOffset: start + end < size ? start + end : undefined },
