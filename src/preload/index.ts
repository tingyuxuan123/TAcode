...
const api: DesktopApi = {
  ...
  window: {
    ...
    setCloseGuardReady: (ready) => ipcRenderer.send("window:close-guard-ready", ready),
    onCloseRequest: (listener) => subscribe("window:close-request", listener),
    answerCloseRequest: (id, allow) => ipcRenderer.send("window:close-answer", id, allow),
  },
  files: {
    ...
    readDrafts: (request) => ipcRenderer.invoke("files:read-drafts", request),
    writeDraft: (request) => ipcRenderer.invoke("files:write-draft", request),
    removeDraft: (request) => ipcRenderer.invoke("files:remove-draft", request),
    inspect: (request) => ipcRenderer.invoke("files:inspect", request),
    mutate: (request) => ipcRenderer.invoke("files:mutate", request),
    location: (request) => ipcRenderer.invoke("files:location", request),
    editors: () => ipcRenderer.invoke("files:editors"),
    open: (request) => ipcRenderer.invoke("files:open", request),
    reveal: (request) => ipcRenderer.invoke("files:reveal", request),
    onMutation: (listener) => subscribe("files:mutation", listener),
    previewUrl: (request) => ipcRenderer.invoke("files:preview-url", request),
    renderHtml: (request) => ipcRenderer.invoke("files:render-html", request),
    releaseHtml: (id) => ipcRenderer.invoke("files:release-html", id),
  ...
  git: {
    ...
    turnSnapshot: (request) => ipcRenderer.invoke("git:turn-snapshot", request.projectRoot),
    onTurnSnapshot: (listener) => subscribe("git:turn", listener),
