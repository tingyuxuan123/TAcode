...
点击左侧 **Skills / MCP**，即可在右侧侧边栏打开两个标签页；也可通过右侧 **+** 菜单分别添加。列表顶部可以切换「当前项目」和「全局」。

Skills 页支持分组卡片、搜索、启停、新建和目录导入。点击卡片可编辑 `SKILL.md`、预览 Markdown、编辑附属文本文件，或将调用命令填入对话。导入会复制完整技能目录，遇到同名目录会跳过；移除会将技能目录放入系统回收站。
...
新建和导入默认写入项目 `.agents/skills` 或全局 `~/.tacode/skills`。停用会将整个技能目录移到同级 `skills-inactive`，启用时移回；共享同一技能目录的其他应用也会受到该操作影响。

## MCP 服务器

**MCP** 页支持本地 stdio、Streamable HTTP 和 SSE。可手动添加、选择常用连接模板，或导入含 `mcpServers` / `servers` 的 JSON。编辑器可配置启动参数、环境变量、请求头、工作目录及超时。启动参数支持每行一个或 JSON 字符串数组；环境变量和请求头支持 `${ENV_VAR}` 引用本机环境变量。

「测试连接」会实际握手并列出可用工具，不执行工具。保存启用后，Agent 会通过现有权限检查调用这些工具；计划模式不开放 MCP 工具。卡片显示的是最近一次测试结果，不代表持续在线。常用连接模板仍需配置对应的本地运行环境或凭据。

项目配置位于 `.tacode/mcp.json`，全局配置位于 `~/.tacode/mcp.json`。项目配置在信任项目后生效，同名服务器覆盖全局项，显式停用也参与覆盖。通过侧边栏保存后，空闲会话自动重载；正在运行的会话在下一轮使用新配置。

Pi 会保留运行中会话的信任决定。如果在会话启动后才首次信任项目，需要完整退出并重启 TACode 才能加载项目技能；之后编辑和启停技能均可正常重载。

运行 `pnpm test:capabilities` 可验证侧边栏操作。它使用临时项目和本地 MCP 测试服务，覆盖编辑、导入、启停、持久化和连接发现。
...
子代理的实际运行状态会同步到侧栏、委派卡片和右侧面板，长命令期间面板持续刷新；需要确认时会在子代理面板显示。可在面板顶部或侧栏会话菜单中选择“停止子代理”。停止或关闭主会话会同时停止所属子代理及其命令；切换到其他会话仍允许原会话在后台运行。
