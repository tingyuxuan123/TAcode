...
## 2026-09-15 FR-14 完成（视觉与交互完整性）

- 新增 `scripts/test-ui-review.mjs`（`pnpm test:ui-review`）：在参考图实测的视口与 DPR（files 820×785、review 836×740，DPR 2，浅色中文）逐状态核对默认/展开/筛选/选中/菜单/窄面板，另含深色映射、中英文、输入法、键盘与无障碍，**18 项通过、16 张截图**（`docs/file-review-reference/fr-14-*.png`）。断言走真实 Chromium 布局与原生指针/键盘，核对结构与尺寸预算，不做像素级叠图。
- 修复两个真实布局缺陷：（1）上下文列里文件树按 `height: 100%` 占满整列，把 AI 审查/意见面板底部操作挤出可视区，改为「文件树吸收剩余高度、面板不收缩、面板内容自己滚动」，并新增面板预算断言（面板不越出列、控件可达）；（2）窄窗内容区被挤到 180px，改为窗口变窄时只收缩**显示宽度**（`可用宽度 − 200`，下限 200），用户拖出的宽度偏好与持久化不变，窗口变宽后恢复。工具栏在窄窗按既有响应式规则换行，其余状态单行且无横向溢出。
- 既有入口核对：脚本断言旧 `FileDrawer` 未挂载（文件入口统一到工作台标签）；浏览器/终端/子会话标签、计划确认卡与 `/undo` 由既有烟测覆盖并重跑（browser、git-review 37 阶段、file-workbench 7 阶段、formats 10 阶段、file-review 5 阶段、真实 App 文档）。
