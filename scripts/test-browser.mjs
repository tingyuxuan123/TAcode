...
try {
  ...
  const env = { ...process.env, TACODE_WORKBENCH_FIXTURE: path.join(output, "renderer/scripts/fixtures/workbench.html") };
  delete env.ELECTRON_RUN_AS_NODE;
  for (const entry of process.env.TACODE_COMPOSER_ONLY ? ["workbench"] : ["smoke", "workbench"]) {
