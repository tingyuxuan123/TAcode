...
const output = await mkdtemp(path.join(tmpdir(), "tacode-file-formats-build-"));
try {
  await build({ entry: { smoke: "scripts/file-formats-smoke.ts" }, format: ["esm"], platform: "node", target: "node22", outDir: output, external: ["electron"], config: false, silent: true, outExtension: () => ({ js: ".mjs" }) });
  ...
  process.exitCode = await new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => child.kill("SIGKILL"), 220_000);
