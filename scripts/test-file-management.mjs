...
const output = await mkdtemp(path.join(tmpdir(), "tacode-file-management-build-"));
try {
  await build({ entry: { smoke: "scripts/file-management-smoke.ts" }, format: ["esm"], platform: "node", target: "node22", outDir: output, external: ["electron"], config: false, silent: true, outExtension: () => ({ js: ".mjs" }) });
